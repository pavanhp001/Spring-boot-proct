...
import com.AL.task.LocalTaskGetOrderByDate;
...
@Component("taskGetOrderByDate")
public class TaskGetOrderByDate extends
		TaskBase<OrderManagementRequestResponseDocument> implements
		LocalTaskGetOrderByDate<OrderManagementRequestResponseDocument> {
	...
	public OrchestrationContext processRequest(
		...
		Calendar moveDate = unmarshallOrder.buildOrderScheduleDate(request);

		getPagingDetails( orderDocument,params );
		try {
			params.add(TaskContextParamEnum.date.name(), moveDate);
	...
	public OrchestrationContext processAgenda(final OrchestrationContext params) {
		Calendar moveDate = (Calendar) params.get(TaskContextParamEnum.date.name());
		...
		List<SalesOrder> saleOrderBeanList = orderManagementDao.findByDate(
				moveDate.getTime(),offSet,totalRows);

		if ((saleOrderBeanList != null) && (saleOrderBeanList.size() > 0)) {
			params.addSuccessful(saleOrderBeanList);
		} else {
			Format formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String s = formatter.format(DateUtil.startOfDay( moveDate.getTime() ));
