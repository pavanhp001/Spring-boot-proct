...
<link rel="stylesheet" href="<c:url value='/style/acdc/acdc_oq_demo.css'/>" />
...
var channelCss = '${channelCss}';
	...
	$(document).ready(function() {
		...
		$('select,input').live("click",symFeedback);
		$(".pc_fldset_data_item .label").removeAttr("style");
...
<form action="<%=request.getContextPath()%>/static/updateDialogue"  method="post" id="frmMain" name="frmMain"	onsubmit="symFeedbackSubmit()" autocomplete=off>
	<input type="hidden" id="CKOPageName" name="CKOPageName" value='oq_demo_content'/>
	...
	<div class="pc_pdetails row">
		<div class="pc_pdetails_logo col-xs-12 col-sm-2 col-md-2">
    		<c:if test="${not empty providerExternalID && !isUtilityOffer}">
    			<img id="provider" src="" 
    		 style="float: left;max-height:60px;max-width:107px;"/> 
	    	</c:if>
  		</div>
  		<div class="pc_pdetails_info col-xs-12 col-sm-2 col-md-8">
		<div class="label">Product Name:</div>
			<c:choose>
			<c:when  test="${productName != null}">
				<div class="pName"><c:out escapeXml="false" value="${productName}" />
				</div><br/>
			</c:when>
			<c:otherwise>
				<div class="pName"><c:out escapeXml="false" value="${productInfo.product.name}" />
				</div><br/>
			</c:otherwise>
			</c:choose>
	...
	<div class="pc_pdetails_cont AL-green">
	<div class="pc_frameblk"> 
			...
			<div id="pageRedirectingButtonsDiv" class="pc_frameblk_right_btns">
				...
				<c:if test="${showBackButton != false}">
					<input id="goToProductCustomization" type="button" class="button" value="< Back" onclick="goBackPage()"/>
				</c:if>
				
				<input id="moveForward" class="button" type="submit" value="Forward >" onclick="return validateDialogue();"/>
			</div>
	 ...
	 <input type="hidden" name="nav_steps_one" class="nav_steps" value="stepTwo"/>
</form>
</div>
</body>
</html>