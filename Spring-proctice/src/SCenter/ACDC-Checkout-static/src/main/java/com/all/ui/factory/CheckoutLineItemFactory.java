...
public enum CKOLineItemFactory {
	...
	INSTANCE;
	private static final Logger logger = Logger.getLogger(CKOLineItemFactory.class);
	/**
	...
	public List<ApplicableType> lineItemPromotionsApplicableType( OrderType orderType, LineItemType orderRecapLineItemType ) {
		List<ApplicableType> promotionList = new ArrayList<ApplicableType>();
		...
		for(LineItemType lineItem:orderType.getLineItems().getLineItem())
		{
			if(isPromotion(lineItem))
			{
				for(Integer applies : lineItem.getLineItemDetail().getDetail().getPromotionLineItem().getAppliesTo())
				{
					if( orderRecapLineItemType.getLineItemNumber() == applies.intValue() )
					{
						promotionList.add(lineItem.getLineItemDetail().getDetail().getPromotionLineItem());
					}
				}
			}
		}
	 ...
	 * @return
	 */
	public  LineItemPriceInfoType buildLineItemPricInfoType(Double baseRecurringPrice, Double baseNonRecurringPrice ){
		LineItemPriceInfoType lineItemPricInfoType  = new LineItemPriceInfoType();
		try{
			if(baseNonRecurringPrice > 0 || baseNonRecurringPrice > 0){
				...
				lineItemPricInfoType.setBaseRecurringPriceUnits("USD");
				lineItemPricInfoType.setBaseNonRecurringPriceUnits("USD");
			}
		}catch(Exception e){
			logger.error("error_occuered_while_build_lineItemPricInfoType",e);
		}
		return lineItemPricInfoType;
	}
	...
	public Map<String,Double> buildFeaturesPriceMap(LineItemType lineItem) 
	{
		Map<String,Double> featuresPriceMap = new HashMap<String,Double>();
		SelectedFeaturesType selectedFeatures = lineItem.getSelectedFeatures();
		...
		if(selectedFeatures != null)
		{
			...
			if(features != null )
			{
				for(FeatureValueType feature:features)
				{
					baseRecurringPrice += feature.getPrice().getBaseRecurringPrice();
					baseNonRecurringPrice += feature.getPrice().getBaseNonRecurringPrice();
				}
			}
		...
		featuresPriceMap.put("baseRecurPrice",baseRecurringPrice);
		featuresPriceMap.put("baseNonRecurPrice",baseNonRecurringPrice);
		return featuresPriceMap;
	}
	
	/** This method return AttributeDetailType value based on attributeName from attributeEntityType
	 * @param attributeEntityType
	 * @param attributeName
	 * @return
	 */
	public String getLineItemAttributeValueBasedOnName(AttributeEntityType attributeEntityType,String attributeName) {
		if(attributeEntityType != null && attributeEntityType.getAttribute()!= null &&  !attributeEntityType.getAttribute().isEmpty()){
			for(AttributeDetailType attributeDetailType:attributeEntityType.getAttribute()){
				if(!Utils.isBlank(attributeName) && attributeName.equalsIgnoreCase(attributeDetailType.getName())){
					return attributeDetailType.getValue();
				}
			}
		}
		return "";
	...
	/**
	 * prepareNewSession method used to create new session and set all the required attributes whenever 
	 * session invalidates due session timeout  
	 * 	
	 * @param sessionVO
	 * @param request
	 * @param util
	 * @throws Exception
	 */
	public void prepareNewSession(SessionVO sessionVO, HttpServletRequest request, JsonUtil<CKOInitialVo> util) throws Exception{
		...
		if(idata!=null && idata.length()>0)
		{
			...
			{
				for(String paramName:CKOVo.getParams()){
					if(paramName.indexOf("=") > 0 && paramName.indexOf("channelCss") == 0){
						request.getSession().setAttribute("channelCss", paramName.split("=")[1]);
					}
					if(paramName.indexOf("=") > 0 && paramName.indexOf("channelType") == 0){
						request.getSession().setAttribute("channelType", paramName.split("=")[1]);
					}
					if(paramName.indexOf("=") > 0 && paramName.indexOf("phoneNo") == 0){
						request.getSession().setAttribute("phoneNo", paramName.split("=")[1]);
					}
				}
				request.getSession().setAttribute("CKOInput", idata);
				request.getSession().setAttribute("lineItemExternalID", CKOVo.getFirstLineItemId());
				request.getSession().setAttribute("orderID", CKOVo.getOrderId());
				request.getSession().setAttribute("lineItemNumber", request.getParameter("lineitemNumber"));
				request.getSession().setAttribute("product_name",request.getParameter("product_name"));
				request.getSession().setAttribute("provider_name",request.getParameter("provider_name"));
				throw new SessionTimeOutException("Exception_Occured_in_CKO_due_to_session_timeout");
			}
		}
