...
public class UpdateDialogueController extends BaseController {
	...
	@RequestMapping(value = "/redirectUpdateDialogue", method = RequestMethod.GET)
	public ModelAndView handleGet(@ModelAttribute("mavRecap") ModelAndView modelAndView, HttpServletRequest request) {		
		...
		if (Utils.isValidSession(sessionVO, SessionKeys.initiator.name())) {
			if (modelAndView != null
				...
				CKOCacheService.INSTANCE.clearMAV(request.getSession().getId(), "redirectUpdateDialogue");
				logger.info("store mav sessionId=" + request.getSession().getId());
				logger.info("sessionID from sessionVO=" + sessionVO.getSessionId());
				CKOCacheService.INSTANCE.storeMAV(modelAndView, sessionVO, "redirectUpdateDialogue");
			} else {
				logger.info("get mav sessionId=" + request.getSession().getId());
				logger.info("sessionID from sessionVO=" + sessionVO.getSessionId());
				modelAndView = CKOCacheService.INSTANCE.getMAV(request.getSession().getId(), "redirectUpdateDialogue");
			}
		}
...
//		modelAndView.setViewName("dialogueUpdate");
	...
	@RequestMapping(value = "/redirectUpdateDialogueError", method = RequestMethod.GET)
	public ModelAndView handleGetError(@ModelAttribute("mavError") ModelAndView modelAndView, HttpServletRequest request) {		
		logger.info("redirectSuccessful");
		modelAndView.setViewName("acdc_error_view");
	  return modelAndView;
	}	
	
	@RequestMapping(value = "/redirectUpdateDialogueSessiontimeOut", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/updateDialogue", method = RequestMethod.POST)
	public String updateDialogue(HttpServletRequest request, final RedirectAttributes redirectAttrs) throws Exception {
...
//			JsonUtil<CKOInitialVo> util = new JsonUtil<CKOInitialVo>();
			...
			mav.addObject("installationVO", Utils.convertJSONString(installationVO));
			mav.setViewName("acdc_installation_info");
			redirectAttrs.addFlashAttribute("mavRecap", mav);
			return "redirect:/static/redirectUpdateDialogue";
		}
		...
		LineItemType newLineItemType = LineItemBuilder.INSTANCE.getLineItemBySelectedDialogues(requestParamMap, availableDataFieldList, availableFeatureMap, availableFeatureGroupMap, orderQualVO.getNewLineItemType(), resultsMap);
		...
		if(updatedOrder.getOrderStatus() != null && updatedOrder.getOrderStatus().getStatus() != null && updatedOrder.getOrderStatus().getStatus().equals("FATAL")){
			...
			redirectAttrs.addFlashAttribute("mavError", mav);
			return "redirect:/static/redirectUpdateDialogueError";
//			return mav;
		}
...
//			return mav;
			...
			if(!Utils.isBlank(page_event) && "DECLINE_AUTHORIZATION".equals(page_event)){
				...
				if(isUtilityOffer == null || !("true").equalsIgnoreCase(isUtilityOffer))
				{
					if (!ishybris && !frontierAsIs)
					{
						 ...
						 */
						mav = ViewUtil.setDigitalView(mav,"acdc_submitLineItem","submitLineItem", channelType, channelCss);
					}
				}
			}
		...
		if("submitLineItem".equalsIgnoreCase(mav.getViewName()) || "acdc_submitLineItem".equalsIgnoreCase(mav.getViewName()))
		{
