	...
	$(document).ready(function() {
    	...
    	$("#submitLineItem").click(function(){
        	...
        	$('#mandatoryDisclaimer').height(30);
     		...
     		$(".error_astrick").remove();
    		$('.mandatory_disclosure_checkbox').each(function(){
    			if($(this).is(":visible")){
    				$(this).attr("alt","visible");
    				if(!$(this).is(":checked")){
    					$(this).next().after("<img style='display: inline-block;height:9px !important; width:9px !important;' class='error_astrick' src='<%=request.getContextPath()%>/image/red_asterisk.png'/>");
    					isUnchecked = true;
    				}
    			}else{
    				$(this).removeAttr("alt");
		...
		<div class="pc_pdetails">
				<div class="pc_pdetails_logo">
			    	<c:if test="${not empty providerExternalID && !isUtilityOffer}">
			    	 <img style="float: left;max-height:60px;max-width:107px;" alt="${lineItemType.lineItemDetail.detail.productLineItem.provider.externalId}" src="${providersImageLocation}${parentExternalId}.jpg"
                     	  onError="this.onerror=null;this.src='${providersImageLocation}${lineItemType.lineItemDetail.detail.productLineItem.provider.externalId}.jpg';"  id="provider" >
				    </c:if>
			  	</div>
						...
						<c:if test="${entity.source == 'SECOND_DESIRED_DATE'}">
							<c:forEach var="attribute" items="${entity.attribute}">
								<c:if test="${attribute.name == 'DATE'}">
									<c:set var="desireDate2" value='${fn:trim(attribute.value)}' />
								</c:if>
											...
											<div class="or_content pagecontainer_or">
												<div class="or_itemdetails">
																...
																<c:forEach var="dataFieldType" items="${dataFieldList}">
																	<c:if test="${dataFieldType.description=='Mandatory'}">
																		...
																		<div style="display: inline-block;">
															                <input id="${dataFieldType.externalId}" class="mandatory_disclosure_checkbox" value="${dataFieldType.externalId}" name="mandatory_disclosure_checkbox" type="checkbox">
													...
													<table cellpadding="0" cellspacing="0" class="or_basemonthlyprice">
														...
														<c:if test="${not empty sessionScope.usageRate}" > 
															<tr class="or_id_row4">
																<td width="3%"></td>
																<td width="30%"><fmt:message key="orderRecap.usageRate" /></td>
																<td width="20%">-</td>
																<td width="47%">
																	<span > <fmt:message key="orderQuick.currency" /><fmt:formatNumber value="${usageRate}" maxFractionDigits="3" pattern="0.00" /></span>
																	<c:if test="${not empty sessionScope.energyRateUnit}" > 
																	  <span class="os_normal">/${energyRateUnit}</span>
																	</c:if>
																</td>
															</tr>
														</c:if>
												...
												<c:if test="${not empty lineItemType.schedulingInfo.desiredStartDate.date || not empty desireDate2}">
													...
													<div class="or_reqinstdate">
														...
														<div>
															...
															<div class="or_reqinstdate_date"><fmt:parseDate var="dateObj" value="${fn:trim(date)}" type="DATE" pattern="yyyy-MM-dd" /><fmt:formatDate pattern="EEEE, MMMM d, yyyy" value="${dateObj}" /> 
																<c:choose>
																	<c:when test="${not empty desireDate1Time}">&nbsp;${desireDate1Time}</c:when>
																	<c:otherwise>
																		<c:if test="${not empty startTime && startTime != '00:00'}">
																			<fmt:parseDate var="startDateObj" value="${fn:trim(date)} ${startTime}" type="DATE" pattern="yyyy-MM-dd HH:mm" />
																			<fmt:parseDate var="endDateObj" value="${fn:trim(date)} ${endTime}" type="DATE" pattern="yyyy-MM-dd HH:mm" />&nbsp;<fmt:formatDate pattern="h:mm a" value="${startDateObj}" />&nbsp;to&nbsp;<fmt:formatDate pattern="h:mm a" value="${endDateObj}" />
																		</c:if>
																	</c:otherwise>
																</c:choose>
															</div>
														...
														<c:if test="${not empty desireDate2}">
															<div>
																...
																<div class="or_reqinstdate_date"><fmt:parseDate var="dateObj" value="${fn:trim(desireDate2)}" type="DATE" pattern="yyyy-MM-dd" /><fmt:formatDate pattern="EEEE, MMMM d, yyyy" value="${dateObj}" />&nbsp;${desireDate2Time}</div>
															</div>
														</c:if>
													</div>
												</c:if> 
																		...
																		<div class="col3">
																			<div class="col3_item">
																				<c:if test="${not empty lineItemType.schedulingInfo.scheduledStartDate.date}">
																					...
																					<fmt:parseDate var="dateObj" value="${fn:trim(date)}" type="DATE" pattern="yyyy-MM-dd" />
