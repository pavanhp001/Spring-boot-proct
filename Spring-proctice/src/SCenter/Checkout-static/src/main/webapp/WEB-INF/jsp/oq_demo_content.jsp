...
var isBackButtonClick = false;
var requiredFields = "<span class='error_message'><img style='display: inline-block;height:9px !important; width:9px !important;'src='<%=request.getContextPath()%>/image/red_asterisk.png'/><span style='color: red; font-size: 12px;font-weight:bold;'> Required Fields</span></span>";
var temp = '${mandatoryDisclosureCheckboxes}';
	function goBackPage(){
	...
	$(document).ready(function() {
		$("input:radio[name=DOMINION-SURGEHELP]").each(function(index,val){
		if(index == 0){
			$(this).attr("checked","checked");
				$("input:radio[name=Dominion_SurgeHelp_ClosingOffer_Enrollment]").each(function(index,val){
					if(index == 0){
						$(this).attr("checked","checked");
					}
					});
	...
	function success() {
	  $("#provider").attr('src','${providersImageLocation}${parentExternalID}.jpg');
	}

	function failure() {
		checkImage('${providersImageLocation}${providerExternalID}.jpg', success1, failure1);
	}
	function success1() {
		  $("#provider").attr('src','${providersImageLocation}${providerExternalID}.jpg');
		}

		function failure1() {
		}
	function checkProdImage(){
		checkImage('${providersImageLocation}${parentExternalID}.jpg', success, failure);
	}
...
<body onload="onLoadFunctions();">
 ...
 <div class="pc_steps_errorblk" style="display: none">
  Number of receivers must equal the number of TVs selected
 </div>
...
<form action="<%=request.getContextPath()%>/static/updateDialogue"  method="post" id="frmMain" name="frmMain"	onsubmit="symFeedbackSubmit()" autocomplete=off>
	...
	<div class="pc_pdetails">
		<c:choose>
    		<c:when test="${not empty providerExternalID && !isUtilityOffer}">
    		<div class="pc_pdetails_logo">
    			<img id="provider" src="${providersImageLocation}${providerExternalID}.jpg" 
    		 style="float: left;max-height:60px;max-width:107px;"/> 
    		   		</div>
	    	</c:when>
	    	<c:otherwise>
	    	<c:if test="${not empty providerExternalID && providerExternalID == 15499441}">
	    	<div class="pc_pdetails_logo">
    			<img id="provider" src="${providersImageLocation}${providerExternalID}.jpg" 
    		 style="float: left;max-height:60px;max-width:107px;"/> 
    		   		</div>
	    	</c:if>
				<div class="pc_pdetails_logo" style="height:5px;">
    		   		</div>
			</c:otherwise>
	    	</c:choose>
	...
	<div class="pc_pdetails_cont">
		...
		<div class="pc_frameblk"> 
			...
			<div id="pageRedirectingButtonsDiv" class="pc_frameblk_right_btns">
				...
				<input id="moveForward" class="buttonClass" type="submit" value="Forward >" onclick="return validateDialogues();"/>
			</div>
...
<input type="hidden" id="receiversList" value="${receiversList}" />
<input type="hidden" id="isReceiverMatch" name="isReceiverMatch" value="${isReceiverMatch}" />
</div>
</body>
</html>