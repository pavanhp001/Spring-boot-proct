...
import static com.AL.ui.util.ConfigProperties.WARMTRANSFER_DIALOGUE_EXTIDS;
...
public enum LineItemBuilder {
	...
	public LineItemType getLineItemBySelectedDialogues(Map<String,String> reqParamMap, 
			...
			Map<String, FeatureGroupType> availableFeatureGroupMap, LineItemType itemType, Map<String, String> resultsMap,HttpServletRequest request){
		...
		String selectedValue = "";
		String extIDSelectedValues = "";
		logger.info("reqParamMap == "+reqParamMap);
		...
		if(reqParamMap.get("extIDSelectedValues") != null && reqParamMap.get("extIDSelectedValues").trim().length()>0){
			extIDSelectedValues = reqParamMap.get("extIDSelectedValues");
		}
		logger.info("extIDSelectedValues == "+extIDSelectedValues);
		if(reqParamMap != null ){
			if(dataFieldList != null && !dataFieldList.isEmpty()){
				...
				logger.info("selValueArray == "+selValueArray);
								
				for(DataField dataFieldValue : dataFieldList){
					
					logger.info("dataFieldValue.getExternalId() "+dataFieldValue.getExternalId());
					logger.info("dataFieldValue.getText() "+dataFieldValue.getText());
					if(dataFieldValue.getFeatureExternalId() == null && selValueArray != null && arrayContainsString(selValueArray, dataFieldValue.getExternalId())){
						...
						if(reqParamMap.get(externalId) != null && reqParamMap.get(externalId).trim().length() > 0){
							...
							logger.info("WARMTRANSFER_DIALOGUE_EXTIDS="+WARMTRANSFER_DIALOGUE_EXTIDS);
							if(!Utils.isBlank(WARMTRANSFER_DIALOGUE_EXTIDS)){
								List<String> dialogueExts = Arrays.asList(WARMTRANSFER_DIALOGUE_EXTIDS.split("\\|"));
								for (String data : dialogueExts) {
									String arr[] = data.split("\\=");
									if(dialogueValueType.getExternalId()!= null && arr[0].equalsIgnoreCase(dialogueValueType.getExternalId())){
										if(!Utils.isEmpty(dialogueValueType.getValue()) && arr[1].equalsIgnoreCase(dialogueValueType.getValue().get(0).getValue())){
											itemType.setEventType(2);
										}
									}
								}
							}
					...
					else if(!Utils.isBlank(extIDSelectedValues) && dataFieldValue.getExternalId().contains("DOMINION-SURGEHELP")&& extIDSelectedValues.contains(dataFieldValue.getExternalId())){
						logger.info("save controller in CKO ");
						dialogueValueTypeValue = oFactory.createDialogValueTypeValue();
						DialogValueType.Value dialogueValueTypeDescriptionValue = oFactory.createDialogValueTypeValue();
						dialogueValueTypeDescriptionValue.setValue(dataFieldValue.getText());
						request.getSession().setAttribute("productDescription", dataFieldValue.getText());
						
						String str= (String)request.getSession().getAttribute("productDescription");
						logger.info("string ==   "+str);
					       dialogueValueTypeValue.setValue(reqParamMap.get("DOMINION-SURGEHELP"));
					       //dialogueValueType = oFactory.createDialogValueType();
					       dialogueValueTypeValue.setSelected(true);
					       ...
					       dialogueValueTypeDescriptionValue.setSelected(true);

							if(dataFieldValue.getBooleanConstraint()!=null){
								dialogueValueTypeDescriptionValue.setType("boolean");	 
							}
							else if(dataFieldValue.getStringConstraint() != null){
								dialogueValueTypeDescriptionValue.setType("string");
							}
							else if(dataFieldValue.getIntegerConstraint() != null){
								dialogueValueTypeDescriptionValue.setType("integer");
							}
					       dialogueValueType.setExternalId(dataFieldValue.getExternalId()+"-NAME");
					       dialogueValueType.getValue().add(dialogueValueTypeDescriptionValue);
					       selectedDialogueTypeDialogue.getDialog().add(dialogueValueType);
					}
				}
