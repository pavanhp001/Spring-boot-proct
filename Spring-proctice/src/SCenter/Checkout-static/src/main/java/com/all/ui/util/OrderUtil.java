...
public enum OrderUtil {
	...
	public ModelAndView buildUtilityProductDialoues(OrderType order,
		...
		stCkVO.setValueByName("simpleChoice.operatingCompany",Utils.getCKOParamValue(CKOVo,"operatingCompany"));
		stCkVO.setValueByName("simpleChoice.operatingCompanyCode",Utils.getCKOParamValue(CKOVo,"operatingCompanyCode"));
		...
		List<Fieldset> fieldsetList = HtmlFactory.INSTANCE.dialogueToFieldSet(
				...
				null, null, preSelectedMap, request.getContextPath(), reuestParamMap, stCkVO, null,orderQualVO.getDominionProductExtIds(),request.getSession().getAttribute("mandatoryDisclosureCheckboxes"));
		...
		if(productInfo.getProductDetails() != null && productInfo.getProductDetails().getMetaData() != null 
			...
			for(String meta : productInfo.getProductDetails().getMetaData().getMetaData()){
				if(meta.equals("OFFER_TYPE=UTILITY")
						|| meta.equals("OFFER_PHASE=ClosingOffer")){
		...
		if(item != null && item.getLineItemDetail() != null 
				&& item.getLineItemDetail().getDetail() != null 
				&& item.getLineItemDetail().getDetail().getProductLineItem() != null){
