...
public enum HtmlFactory {
	...
	public List<Fieldset> dialogueToFieldSet(StringBuilder events,
			...
			StaticCKOVO stCkVO, Map<String, List<PriceDisplayVO>> priceDisplayVOMap, String dominionProductExtId,Object mandatoryDisclosureCheckboxes) throws Exception{
		...
		for (Dialogue dialogue : dialogueVO.getDialogueNameList()) {
			...
			for(DataGroup dGroup : dialogue.getDataGroupList()){
				for(DataField df : dGroup.getDataFieldList()){
					if(df.getExternalId().contains("DOMINION-SURGEHELP") && !Utils.isBlank(dominionProductExtId) && !dominionProductExtId.contains(df.getExternalId())){
						continue;
					}
					/*
						--------------------------------------------------------------------------------------------------------------------
					...
					if(df.getInfoType() != null){
						if(df.getInfoType().equalsIgnoreCase("DISCLOSURE") || df.getInfoType().equalsIgnoreCase("notable") || df.getInfoType().equalsIgnoreCase("informational")){
							...
							if(df.getInfoType().equalsIgnoreCase("DISCLOSURE")){
								Span spanMandatory = oFactory.createSpan();
								spanMandatory.setStyle("color:red; font-weight:bold;");
								spanMandatory.getContent().add("MANDATORY ");
								disclosureLabel.setClazz("disclosure_type_label");
								disclosureLabel.getContent().add(spanMandatory);
							...
							if(df.getInfoType().equalsIgnoreCase("DISCLOSURE")){
								...
								//checkbox.setName("mandatory_disclosure_checkbox"+df.getExternalId().replace("/", "_"));
								checkbox.setName("mandatory_disclosure_checkbox");
								checkbox.setId(df.getExternalId().replace("/", "_"));
								checkbox.setValue(df.getExternalId().replace("/", "_"));
								checkbox.setClazz("mandatory_disclosure_checkbox");
								if(mandatoryDisclosureCheckboxes != null){
									String mandatoryDisclosure[] = mandatoryDisclosureCheckboxes.toString().split(",");
									for(int i =0 ;i<mandatoryDisclosure.length;i++){
										if(mandatoryDisclosure[i].trim().equalsIgnoreCase(df.getExternalId().replace("/", "_"))){
					 ...
					 */
						...
						if(df.getDisplayGroup() != null){
							...
							dialogueLabelSpan.setStyle("color:grey; padding-left: 5px;");	
						}
						...
						/*txtFieldsDiv.getClazz().add("dominionTier_content");
						...
						boolean isDominion = false;
						Span dialogueValueSpan = oFactory.createSpan();
						dialogueValueSpan.getClazz().add("value dominionTier");
						if(!Utils.isBlank(df.getExternalId()) && df.getExternalId().contains("DOMINION-SURGEHELP")){
							dialogueValueSpan.setStyle("float:left; padding-left:30px;");
							inputDisplayType = InputTypeEnum.radio;
							isDominion = true;
							createBooleanFieldsForDominion(df,dialogueValueSpan,dGroup.getName(),dominionProductExtId);	
						}
	...
	private void createBooleanFieldsForDominion(DataField df, Span fieldset,String dataGroupName ,String dominionProductExtId) throws Exception {
     if(dominionProductExtId != null && dominionProductExtId.contains(df.getExternalId())){
		...
		Input inputY = oFactory.createInput();
		inputY.setType(InputTypeEnum.radio.name());
		inputY.setName("DOMINION-SURGEHELP");
		inputY.setClazz(DEFAULT_CSS_CLASS+" "+RADIO_LABEL_STYLE);
		if(df.getFeatureExternalId() != null){
			//String externalId = df.getFeatureExternalId().substring(df.getFeatureExternalId().length()-1);
			inputY.setName("DOMINION-SURGEHELP");
			inputY.setClazz(DEFAULT_CSS_CLASS+" "+RADIO_LABEL_STYLE+" Feature"+" FeatureCheck");
		}
		inputY.setAlt(df.getExternalId());
		//inputY.setId(mainString);
		inputY.setId("BOOL_Y_" + dataGroupName +"_" + mainString);
		logger.info("StringConstraint value ="+df.getStringConstraint().getListOfValues().get(0));
		inputY.setValue(df.getStringConstraint().getListOfValues().get(0));
		rSpan.getContent().add(inputY);
	...
	private Span addDialogueFeatureField(FeatureType features, List<String> viewDetailsFeaturesList, Map<String, String> preSelectedMap, 
		...
		boolean isStringConstrain = false;
		//DOMINIONHELP_TIER
	...
	private static void buildTextTypeDialogues(InputTypeEnum inputType, String mainString, DataField df, 
		...
		if("consumer.dateOfBirth".equalsIgnoreCase(df.getValueTarget())){
			input.setPlaceholder("mm/dd/yyyy");
		}
	...
	private Input createHiddenInputCheckinBoxForDominion(FeatureType feature){

		if(feature.getDataConstraints().getStringConstraint() !=  null){
			Input iChkBoxHidden = oFactory.createInput();
			iChkBoxHidden.setType("hidden");
			iChkBoxHidden.setId(feature.getExternalId()+"_Feature_price");
			iChkBoxHidden.setValue(feature.getDataConstraints().getStringConstraint().getValue().split("|")[0]);
			return iChkBoxHidden;
		}
		return null;
	}
	...
	private void addTableDataFeaturesField(FeatureType features, Tr tr, List<String> previousCKOSelFeatureList, 
		...
		if(features.getIncluded() != null){
			...
			if(Utils.isBlank(selectedFeatureJSON)){
				...
				if(providerSessionData != null && providerSessionData.get(providerExternalID) != null && !Utils.isBlank(providerExternalID)){
					selectedFeatureJSON = providerSessionData.get(providerExternalID).get("selectedFeaturesJSONHiddenValue");
				}
				logger.info("session_provider_Selected_Features_String="+selectedFeatureJSON);
			}
