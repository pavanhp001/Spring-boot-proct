...
public class RepositoryAmazonItTicketS3Impl implements RepositoryItTicketService 
{
	private static final Logger logger = Logger.getLogger(RepositoryAmazonItTicketS3Impl.class);
	...
	public RepositoryAmazonItTicketS3Impl(AmazonS3 client) 
	{
		s3 = client;
	}
	...
	{	
		...
		S3Object obj = s3.getObject(new GetObjectRequest(bucket, getS3Key( "" ) + name));
	...
	public String putObject(String name, InputStream is) 
	{
		...
		if(s3.doesBucketExist( bucket ))
		{
			...
			name = (key.contains("prod")?"concert":key)+"_"+name;
			PutObjectResult result = s3.putObject(new PutObjectRequest(bucket, getS3Key( "" ) + name, is, meta));
