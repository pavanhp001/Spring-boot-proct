function goToConfirm(){
	try{
		...
		var isMoving = $('select#moveindateYesNo').val();
		...
		*/
		...
		if($('select#moveindateYesNo').is(":visible")){
			if(isMoving==null || isMoving == "" || isMoving == "Moving?"){
				$('select#moveindateYesNo').after($('<span class="missingFields">*</span>'));
				 stat = false;
			}
		}
		...
		if($('input#datepicker').is(":visible"))
		{
			...
			if(moveDate.length < 10 && moveDate.length!=0){
				...
				alert("Invalid Move-In Date format.  Please enter date in mmddyyyy format.(OR) The date is not a valid date");
				return false;
			}
			else
			{
...
/*
 * Peco account number validation....
 * 
 * 
 

function pecoAccountNumberValidation(){
	var pecoAccountNumber = $('input#pecoAccountNumber').val();
	var statusForPecoAccNum = true;
	if(pecoAccountNumber!=null && pecoAccountNumber!=""){
		if(pecoAccountNumber.length==11){
			var splittedPecoAccNum = pecoAccountNumber.split("-");
			if(splittedPecoAccNum.length==2){
				if(isNaN(splittedPecoAccNum[0]) || isNaN(splittedPecoAccNum[0]) ){
					$('#errorForPecoNum').text('Invalid PECO Account Number');
					$('#errorForPecoNum').css('color','#F00');
					$('#errorForPecoNum').css('display','inline');
					statusForPecoAccNum = false;
				}
			}else{
				$('#errorForPecoNum').text('Invalid PECO Account Number');
				$('#errorForPecoNum').css('color','#F00');
		...
		$('input#pecoAccountNumber').after($('<span class="missingFields">*</span>'));
		statusForPecoAccNum = false;
	}
	return statusForPecoAccNum;
}

// Auto format for Peco Account number....

function pecoNumberFormat(value){
	if(value.length>=6){
		if(value.indexOf("-")==-1){
			var formatedPecoNumber = value.substring(0,5)+"-"+value.substring(5,value.length);
			$('input#pecoAccountNumber').val(formatedPecoNumber);
		}
	}
}
*/
