...
function populateFields() {
	...
	$("input[name='Discovery_New_NonRR_1']").val("<c:out value="${questionList['#TVS']}"/>");
	$("input[name='Discovery_New_NonRR_2']").val("<c:out value="${questionList['#HD']}"/>");
	$("input[name='Discovery_New_NonRR_3']").val("<c:out value="${questionList['#DVR']}"/>");
	
	if(questionsList!=undefined && questionsList["Channels"]!=undefined){
		$("input[name='Discovery_New_NonRR_4']").val(questionsList["Channels"]);
	}else{
		$("input[name='Discovery_New_NonRR_4']").val("");
	}
	
	$("input[name='Discovery_New_NonRR_5'][value='<c:out value="${questionList['Movie Channels']}"/>']").attr('checked', 'checked');
	$("input[name='Discovery_New_NonRR_6'][value='<c:out value="${questionList['Internet']}"/>']").attr('checked', 'checked');

	if(questionsList!=undefined && questionsList["Internet Usage"]!=undefined){
		$("input[name='Discovery_New_NonRR_7']").val(questionsList["Internet Usage"]);
	}else{
		$("input[name='Discovery_New_NonRR_7']").val("");
	}
	
	$("input[name='Discovery_New_NonRR_8']").val("<c:out value="${questionList['#Devices']}"/>");
	$("input[name='Discovery_New_NonRR_9'][value='<c:out value="${questionList['Home Phone']}"/>']").attr('checked', 'checked');
	}catch(err){alert(err);}
}

function backToQualification(){
	var path = $("input#contextPath").val();
	var action = path +"/salescenter/qualification";
	$("form#discovery").attr("action", action);
...
span.noborder p{
	margin: 0;
					...
					<section id="contentfullcont">
						...
						<section id="content">
							<form id="discovery" name="discovery "action="<%=request.getContextPath()%>/salescenter/recommendations" method="post" autocomplete="off">
							...
							<div class="contentwrapper">
								...
								<div class="bottombuttons">
									<div class="rightbtns">
										<input type="button" id="discoveryNewBack" name="discoveryNewBack" value="< Back" onclick="backToQualification();" />
