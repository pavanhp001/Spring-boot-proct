...
{
 var existingChecked = $("#existingService").attr("checked");
 var newChecked = $("#newService").attr("checked");
 if (!existingChecked && !newChecked){
          //document.getElementById("existingService").checked = true;
		 //alert('none checked::'+document.getElementById("existingService").checked);
	 }
   return true;
}
...
$(document).ready(function(){
	$('#newService').click (function (){
		...
		if (thischeck.is(':checked'))
		{
			$("#newMsgBlk").show();
			$("#serviceType").val("new");
			$('#existingService').attr('checked', false);
			$("#existingMsgBlk").hide();
			$('#existingYesService').attr('checked', false);
			$("#existingYesMsgBlk").hide();
			$('#existingNoService').attr('checked', false);
			$("#existingNoMsgBlk").hide();
		}else{
			$("#newMsgBlk").hide();
			$('#existingService').attr('checked', false);
			$("#existingMsgBlk").hide();
			$("#serviceType").val("");
			$("#serviceTransferType").val("");
		}
	});
	$('#existingService').bind("click", function (){
		$("#errorMessage").html("");
		var thischeck = $(this);
		if (thischeck.is(':checked'))
		{
			$("#existingMsgBlk").show();
			$("#serviceType").val("transfer");
			$('#existingYesService').attr('checked', false);
			$("#existingYesMsgBlk").hide();
			$('#existingNoService').attr('checked', false);
			$("#existingNoMsgBlk").hide();
			$('#newService').attr('checked', false);
			$("#newMsgBlk").hide();
		}else{
			$("#existingMsgBlk").hide();
			$("#existingYesMsgBlk").hide();
	...
	$('#existingYesService').click (function (){
		var thischeck = $(this);
		if (thischeck.is(':checked'))
		{
			$("#existingYesMsgBlk").show();
			$("#serviceTransferType").val("yes");
			$("#existingNoMsgBlk").hide();
			$('#existingNoService').attr('checked', false);
		}else{
			$("#existingYesMsgBlk").hide();
	...
	$('#existingNoService').click (function (){
		var thischeck = $(this);
		if (thischeck.is(':checked'))
		{
			$('#existingYesService').attr('checked', false);
			$("#existingYesMsgBlk").hide();
			$("#existingNoMsgBlk").show();
			$("#serviceTransferType").val("no");
		}else{
			$('#existingYesService').attr('checked', false);
			$("#existingYesMsgBlk").hide();
			$("#existingNoMsgBlk").hide();
	...
	var typeOfService = '${typeOfService}';
	 if (typeOfService == 'newService') {
		 $("#newService").attr("checked", "checked");
		 $("#newService").trigger("click");
		$("#newService").attr("checked", "checked");
	} else if (typeOfService == 'existingService') {
		$("#existingService").attr("checked", "checked");
		$("#existingService").trigger("click");
		$("#existingService").attr("checked", "checked");
		var val = '${typeOfcheckBox}';
		if ("${typeOfcheckBox}" == "Y") {
			$("#existingYesService").attr("checked", "checked");
			$("#existingYesService").trigger("click");
			$("#existingYesService").attr("checked", "checked");
		} 
		if("${typeOfcheckBox}" == "N") {
			$("#existingNoService").attr("checked", "checked");
			$("#existingNoService").trigger("click");
			$("#existingNoService").attr("checked", "checked");
		}
	}
...
.qualificationformcontent {
    ...
    height: 173px;
					...
					<section id="contentfullcont">
						...
						<section id="content">
							<form id="qualification" name="qualification "action="<%=request.getContextPath()%>/salescenter/discovery" method="post">
								...
								<div id="action_wrapper">
									<div class="widget_container">
										<div class="pagecontainer" align="center">
											<!-- Form Content -->
											<section class="qualificationformcontent" >
												<div style="text-align:center;">
													<span style="padding-right:50px;">
														<input type="checkbox" id="newService" name="newService" style="vertical-align: text-top;margin-right:5px;" value="newService" /><fmt:message key="qualification.newservices" />
													</span>
													<span>
														<input type="checkbox" id="existingService" name="existingService" style="vertical-align: text-top;margin-right:5px;" value="existingService"/><fmt:message key="qualification.transferexistingservices" />
													</span>
													<br/>
													<div id="newMsgBlk" class="displayNone" style="padding:10px; text-align:justify;">
														<span>${order.customerInformation.customer.firstName}&nbsp;${order.customerInformation.customer.lastName}</span><SPAN><fmt:message key="qualification.newdialogue" /></SPAN>
													</div>
													<div id="existingMsgBlk" class="displayNone" style="padding:10px; text-align:justify;">
														<div style="padding-bottom:10px;">
															<fmt:message key="qualification.transferexistingservicesdialogue" />
														</div>
														<div style="text-align:center;">
															<span style="padding-right:50px;">
																<input type="checkbox" id="existingYesService" name="yesTransferService" style="vertical-align: text-top;margin-right:5px;" value="Y"/>
																<fmt:message key="qualification.transferyes" />
															</span>
															<span style="padding-right:50px;">
																<input type="checkbox" id="existingNoService" name="noTransferService" style="vertical-align: text-top;margin-right:5px;" value="N"/>
																<fmt:message key="qualification.transferno" />
															</span>
														</div>
													</div>
													<div id="existingYesMsgBlk" class="displayNone" style="padding:10px;margin-top: -20px; text-align:justify;">
														<fmt:message key="qualification.transferyesdialogue" />			
													</div>
													<div id="existingNoMsgBlk" class="displayNone" style="padding:10px; margin-top: -20px; text-align:justify;">
														<fmt:message key="qualification.transferynodialogue" />	
													</div>
												</div>
