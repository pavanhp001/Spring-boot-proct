...
<style type="text/css">
.pc_fldset_data_item span.label{
	width: 576px;
}
</style>
...
$(document).ready(function(){
	try{
		$('.tpv_transfer').change(function(){
		    if(this.value == 'No'){
		        $('#TPV_Completed').prop('disabled', 'disabled');
		        $('#TPV_Completed option[value="No"]').attr("selected",true);
		    }
		    else if(this.value == 'Yes'){
			    if($('#TPV_Completed').is(':disabled')){
			        $('#TPV_Completed').prop('disabled', false);
			    	$('#TPV_Completed option[value="Yes"]').attr("selected",true);
				}
			}
		});
...
function validateDialogue(){
	...
	$('select').each( function() {
		...
		if($(this).is(':visible') && $(this).val() == ""){
			$('#'+id).after("<span id='"+id+"_validate'><font color='red' style='font-size: 18px;font-weight:bold;' > *</span>");
					...
					<section id="contentfullcont">
						<header id="content_header">
							<div class="row">
								...
								<span class="cell pageTitle">TPV</span>
								<span class="callTime" id="startTimeText"></span>
							</div>
						</header>
						<section id="content">
							<form id="tpv" name="tpv" action="<%=request.getContextPath()%>/salescenter/tpvSave" method="post" onsubmit='savePageHtml(false, "");' >
							<input type="hidden" id="submitSt" name="submitSt"/>
							<input type="hidden" id="contextPath" value="<%=request.getContextPath()%>" />
							<input type="hidden" id="pageTitle" name="pageTitle" value="TPV">
							<input type="hidden" id="extIds" name="extIds" />
							<input type="hidden" id="urlPath" name="urlPath" value="<%=request.getSession().getAttribute("urlPath")%>">
							<input type="hidden" id="orderId" value="<%=request.getSession().getAttribute("orderId") %>" name="orderId" />
							<input type="hidden" id="tpvLineItemId" name="tpvLineItemId" value="${tpvLineItemId}">
							<input type="hidden" value="${providerExternalId}" id="provider_Id" name="providerId" />
							<input type="hidden" value="${productExternalId}" id="prodExtId" name="prodExtId"/> 
							<div class="pc_pdetails_cont">
									...
									<div class="pc_frameblk" style="max-height:534px;">
										...
										<div id="errorMessage" style="display: none;">
											<span style="background-color:yellow;"><font color="red" style="font-weight: bold;font-size: 14px;">* Required field.</font></span>
										</div>
									</div>
								</div>
								<div class="bottombuttons">
									<div class="rightbtns" style="padding-top: 47px;">
										<input  type="submit" id="tpvForward" value="Forward >"  onclick="return validateDialogue();"/>
									</div>
								</div>
							</form>
						</section><!--  Content -->
