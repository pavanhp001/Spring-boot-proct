...
#customerTable.table, #customerTable td, #customerTable th
{
...
$("input[name='consumerSearch']").click(fnOpenConsumerPopup);
$("input[name='cancelSearch']").click(closeConsumerPopup);
$('#consumerFind').click(function(){
	   if(($.trim($('#consumerSearchFirstName').val()) != '' && $.trim($('#consumerSearchLastName').val()) != '') || ($.trim($('#consumerSearchZip').val()) != '' && $.trim($('#consumerSearchLastName').val()) != '')){
		   $("#consumerRequired").css("color", "grey");
		   var referrerIdValue = document.getElementById("referrerIdValue").value;
		   var firstName = document.getElementById("consumerSearchFirstName").value;
		   var lastName = document.getElementById("consumerSearchLastName").value;
		   var zipCode = document.getElementById("consumerSearchZip").value;
		   var data = {"referrerIdValue" : referrerIdValue, "firstName" : firstName, "lastName" : lastName, "zipCode" : zipCode};
		    $.ajax({
		    	type: 'POST',
		    	data:data,
		    	url: "<%=request.getContextPath()%>/salescenter/searchCustomerLookup",
		    	success: function(data1){
		    		data1 = JSON.parse(data1);
		    		if(data1.customerLookups=="none"){
		    			document.getElementById("customerSearchResultsTable").innerHTML = "<tr style='background-color: grey; width:100%'><td colspan='2'><b>No Results</b></td></tr>";
		    		} else {
		    			var tableResults = "<tr style='background-color: grey; width:100%'><td><b>First Name</b></td><td><b>Last Name</b></td><td><b>Address</b></td><td>&nbsp;</td>";
		    			var str_array = data1.customerLookups.split(',');
		    			for(var i = 0; i < str_array.length; i++) {
		    				str_array[i] = str_array[i].replace(/^\s*/, "").replace(/\s*$/, "");
		    					if (str_array[i]) {
		    						if(i%4 == 0) {
		    						tableResults = tableResults + "</tr><tr>";
		    					}
		    				    tableResults = tableResults + "<td>" + str_array[i] + "</td>";
		    					}
		    				}
		    
		    			document.getElementById("customerSearchResultsTable").innerHTML = tableResults;
		   ...
		   $('div#customerSearchResults').css("display","block");     
	   } else {
		   $("#consumerRequired").css("color", "red");
	   }

	});
});
function submitCustomerLookupSearch(id) {
	var defer = $.Deferred();
	defer.resolve(savePageHtml(true, ""));
	defer.promise();
	document.getElementById("customerLookupSelectId").value = id;
	$("#consumer-popup").dialog('close');
	submitCall();
}
function submitCall(){
	try{
		setTimeout(function(){
			document.getElementById("greeting").submit();
       },900);
	}catch(err){alert(err);}
}
...
var fnOpenConsumerPopup = function(){

	try{
    $("#consumer-popup").dialog({
        resizable: true,
...
var closeConsumerPopup = function(){
	savePageHtml(true, "");
	setTimeout(function(){
		$("#consumer-popup").dialog('close');
	}, 300);
...
$(window).on('beforeunload', function(){
	$.blockUI({ message: $('#domMessage') }); 
					...
					<section id="contentfullcont">
									...
									<div class="widget_container">
										<div class="pagecontainer" align="center">
											...
											<!-- Form Content -->
												...
												<input type="hidden" id="customerLookupSelectId" name="customerLookupSelectId" value="">
												...
												<div id="customerTableDiv" style="height: 200px;overflow-y: auto;">
													<table id="customerTable" class="customerTable" style="width:500px;border:1px solid #d3d3d3;" cellpadding="2" cellspacing="1" border="1" >
														<tbody>
															<c:forEach var="cust" items="${customerVoList}">
											...
											<div style="width:785px;text-align:right; " id="refreshBtn">
													&nbsp;&nbsp;&nbsp;<input type="button" id="consumerSearch" name="consumerSearch" value="Customer Search" style="margin-top:5px;padding:3px;"/>&nbsp;
						...
						<div id="consumer-popup" style="display:none;">
								<p class="black">Type in name and zipcode to find customer</p>
								<input id="referrerIdValue" name="referrerIdValue" type="hidden" value="${referrerIdValue}"/>
								<input id="consumerSearchFirstName" name="consumerSearchFirstName" type="text" title="First Name" placeholder="First Name">
								<input id="consumerSearchLastName" name="consumerSearchLastName" type="text" title="Last Name" placeholder="Last Name">
								<input id="consumerSearchZip" name="consumerSearchZip" size="7" type="text" title="Zip" placeholder="Zip">		
								<input id="consumerFind" name="consumerFind" value="Find" type="button"><input id="cancelSearch" name="cancelSearch" value="Cancel" type="button"><br/>						
								<font size="2" color="grey" id="consumerRequired" name="consumerRequired"><i>*Required First Name and Last Name OR Last Name and Zip</i></font>
								<br/><br/>
								<div id="customerSearchResults" name="customerSearchResults" style="display:none;">
													<table style="width:100%;border:1px solid #d3d3d3;" cellpadding="2" cellspacing="1" border="1" >
														<tbody style="width:100%" id="customerSearchResultsTable">
														</tbody>
													</table> 
								</div>												
						</div>						
