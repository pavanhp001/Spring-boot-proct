...
<script src="<c:url value='/script/basic_information_peco.js'/>"></script>
...
$(document).ready(function(){
 ...
 replaceTextWithSelector("p",["{consumer.dwelling.addressBlock}",
	 "{consumer.name.nameBlock}"],["<Address>","<Customer Name>"]);
    function replaceTextWithSelector(selector,
	  oldStringArray,newStringArray){
	    try{
		    var oldArryIndex = 0;
             $.each(newStringArray,function(key, newValue) {
                     $(selector).text(function(index, text){  
                  return text.replace(oldStringArray[oldArryIndex], newValue); 
                 }); oldArryIndex++;
              });
        }
        catch(err){alert(err);}
...
*/
	    ...
	    */
        ...
        var namePrefix = JSON.parse('<c:out value="${namePrefix}" escapeXml="false"/>');
        var nameSuffix =  JSON.parse('<c:out value="${nameSuffix}" escapeXml="false"/>');
...
function submitForm(){
	for(var i = 0; i < textFldIds.length; i++) {
	    if (textFldIds[i] === $(this).attr("id")) {
	    	if($(this).val() == ""){
	    		if(!$(this).next().is('label') && $(this).is(':visible')){
	    	    	$(this).after("<label style='color:red;font-size:18px;'>*</label>");
	    		}
	    	}else{
	    	    var element = $(this).next();
		...
		$.blockUI({ message: $('#domMessage') }); 
					...
					<section id="contentfullcont">
						...
						<section id="content">
				            ...
				            <input type="hidden" id="selectedState" value='<c:out value="${salescontext.scMap['address.state']}" escapeXml="false"/>'>
				            <input type="hidden" id="consumerPrefix" value='<c:out value="${salescontext.scMap['consumer.name.prefix']}" escapeXml="false"/>'>
				            <input type="hidden" id="consumerSuffix" value='<c:out value="${salescontext.scMap['consumer.name.suffix']}" escapeXml="false"/>' >
				            <input type="hidden" id="selectedUnitType" value='<c:out value="${salescontext.scMap['unit.type']}" escapeXml="false"/>' >
											...
											<div class="basicinfoformcontent">
												<div class="fieldsrow">
													...
													<div class="firstnmfield">
														<input id="firstName" name="firstName" type="text" class="text" value='<c:out value="${salescontext.scMap['consumer.name.first']}"/>' maxlength="100" onchange="capitalizeText(this);" title="First Name" placeholder="First Name" />
													</div>
													<div class="middlenmfield">
														<input id="middleName" name="middleName" type="text" class="text" value='<c:out value="${salescontext.scMap['consumer.name.middle']}"/>' onchange="capitalizeText(this);"  title="MI" size="2"  maxlength="1"  placeholder="MI" />
													</div>
													<div class="lastnmfield">
														<input id="lastName" name="lastName" type="text" class="text" value='<c:out value="${salescontext.scMap['consumer.name.last']}"/>' onchange="capitalizeText(this);"  maxlength="100" title="Last Name" placeholder="Last Name"/>
												...
												<div class="fieldsrow">
													<div class="addressfield">
														<input id="address1" name="address1" type="text" class="text addressfield" value='<c:out value="${salescontext.scMap['address.street1']}"/>' onchange="capitalizeText(this);"  maxlength="100" title="Address 1"  placeholder="Address 1"/>
													</div>
												</div><!-- Row -->
												<div class="fieldsrow">
													...
													<div class="unitnofield">
														<input id="unitNumber" name="unitNumber" type="text" class="text" value='<c:out value="${salescontext.scMap['unit.number']}"/>' title="Unit Number" size="10" placeholder="Unit Number" />
													</div>
												</div><!-- Row -->
												<div class="fieldsrow">
													<div class="cityfield">
														<input id="city" name="city" type="text" class="text" value='<c:out value="${salescontext.scMap['address.city']}"/>' onchange="capitalizeText(this);" maxlength="100" title="City"   placeholder="City"/>
													</div>
													...
													<div class="zipfield">
														<input id="zipcode" name="zipcode" type="text" class="text" value='<c:out value="${salescontext.scMap['address.zip']}"/>' maxlength="10" title="Zip Code" placeholder="Zip Code" />
													</div>
												...
												<div class="fieldsrow" style="display: none">
													...
													<div class="moveindatefield" id="moveInDateDivId">
														&nbsp;&nbsp;&nbsp;<input type="text" id="datepicker" maxlength="10" name="datepicker1" value=''  title="Move-In Date" placeholder="Move in Date" />
													</div>
												</div>
												<%-- <div class="fieldsrow">
													<div class="moveindatefield" id="moveInDateDivId">
														<input type="text" id="datepicker" maxlength="10" name="datepicker1" value='<c:out value="${dob}"/>' title="Move-In Date" placeholder="Move in Date" />
													</div>
												</div> --%><!-- Row -->												
											</div>
											<!-- Form Content -->
									...
									<div>
										<div>
										    <div style="margin-top: 296px">
												<div class="leftbtns" style="margin-left: 215px; margin-top: -18px">
													<div class="row" id="validatorMsg" style="display:none; color:#F00; margin: 35px 0px 0px 0" >
