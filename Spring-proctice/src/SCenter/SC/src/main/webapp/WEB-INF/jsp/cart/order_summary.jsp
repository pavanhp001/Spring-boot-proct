...
<script src="<c:url value='/script/order_summary.js'/>"></script>
 ...
 $(document).ready(function() {
		if(!isNaN($("#hiddenBaseMonthlyNonPromotionPrice").val())){
			...
			if(baseMonthlyNonPromotionValue<0){
				$("#baseMonthlyNonPromotionPriceTdId").text("TBD");
			}else{
				$("#baseMonthlyNonPromotionPriceTdId").text("$"+(baseMonthlyNonPromotionValue.toFixed(2)));
			}
		}
	});
 */
...
<body onload="updateTimer();">
	<form method="POST" action="<%=request.getContextPath()%>/rest/recap/${order.externalId}" name="ordersummary" id="ordersummary">
		...
		<input type="hidden" id="orderId" value="${order.externalId}">
							...
							<c:set var="productFeatureMap" value='${productFeatureNameMap[productExternalId]}'/>
										...
										<c:if test="${liStatus != 'CANCELLED_REMOVED'}">
											...
											<input type="hidden" name="productSource" value="${productSource}" />
														...
														<!--CKO and AddToOrder-->
															<c:if test="${liStatus != 'CANCELLED_REMOVED'}">
																<c:if test="${statusAttribute != 'CKOComplete'}">
																	...
																	onclick="CKO(${lineitem.lineItemExternalId}, ${providerId},'${lineItemProviderName}', '${productSource}');"
																	/>
														...
														<!--Provider Logo-->
															<a href="#"> 
																<img alt="${lineitem.providerExtId}" src="${providersImageLocation}${parentExternalId}.jpg"
																				onError="this.onerror=null;this.src='${providersImageLocation}${lineitem.providerExtId}.jpg';" style="float: left;">
															</a>
														</div>
													</div>
														...
														<div class="os_itemnameinfo">
															...
															<c:if test="${liStatus != 'CANCELLED_REMOVED'}">
																<table cellpadding="0" cellspacing="0" class="os_itemdetailstbl">
																	...
																	<c:choose>
																		<c:when test="${productSource == 'REALTIME'&& (providerId!=32416075 || providerId!=15499341 || providerId!=15499381)}">
																			<tr class="os_id_row2">
																	...
																	<tr class="os_id_row3" style="font-weight: bold;">
																		...
																		<td></td>
																		<td style="text-align: right;"><fmt:message key="orderQuick.currency" /><fmt:formatNumber value="${lineitem.monthlyTotal}" maxFractionDigits="2" pattern="0.00"/></td>
																	</tr>
																	<!--Installation Fee-->
																		...
																		<div class="os_promotions">
																			...
																			<c:forEach var="applicableType"	items="${promotionMap.value}">
																				<div class="os_promo_desc">
																					${applicableType.promotion.shortDescription}
																					<c:if test="${empty applicableType.promotion.shortDescription}">
																					...
																					<c:if test="${productSource == 'REALTIME'}">
																						...
																						<font color="red">
																							...
																							<c:if test="${empty monthlyPriceAfterPromoEndsValue && (providerId!=32416075 || providerId!=15499341 || providerId!=15499381)}">
													...
													<div class="os_statusdata">
														...
														<c:if test="${lineitem.lineItemStatus != 'CANCELLED_REMOVED'}">
														<c:choose>
																<c:when test="${providerId == '27010360' || providerId == '15500201'|| providerId == '24699452'|| providerId == '32416075' || providerId == '15499341' || providerId == '15499381'}">
																	<c:if test="${statusAttribute != 'CKOComplete'}">
								...
								<div class="bottombuttonsOrderSummary">
									<div class="leftbtns">
										...
										<input type="button" id="placeOrder" name="orderRecap"   value="<fmt:message key="orderSummary.placeOrder.button"/>" />
									</div>
								</div>
