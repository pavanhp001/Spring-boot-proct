...
<head>
<title><fmt:message key="utilityoffer.header" /></title>

<script type="text/javascript">
function backToOffer(){
	var href = window.location.href;
	window.location = href.slice(0, href.lastIndexOf("/")+1) + "offer";
}

function goToQualification(){
	$("form#utilityoffer").submit();
}
...
#content_header div span:nth-of-type(3) {
    display: inline;
    float: right;
    font-size: 12px;
    left: 0;
...
<header id="content_header">
	<div class="row">                	
		...
		<input type="hidden" id="callStartTime" value="<%=request.getSession().getAttribute("callStartTime") %>" name="callStartTime" />
   		<span class="cell"><fmt:message key="utilityoffer.header" /></span>
   		<span class="cell" id="startTimeText"></span>
	</div>
</header>

<div class="content_border">
	<form id="utilityoffer" name="utilityoffer "action="<%=request.getContextPath()%>/salescenter/qualification" method="post">
	...
	<footer id="action_footer">
        <span class="row  bottomDweller">
               <input type="hidden" id="pageTitle" name="pageTitle" value="Utility Offer">
               <input type="button" value="backToOffer" id="backToOffer" class="backArrowBtn" onclick="backToOffer();"/>
               <input type="button" value="goToQualification" id="goToQualification" class="forwardArrowBtn" onclick="goToQualification();"/>   
        </span>
	</footer>
</div>
