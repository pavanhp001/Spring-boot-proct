...
<link rel="stylesheet" type="text/css" href="<c:url value='/css/css_new/recommendations.css'/>" />
    ...
    $.ajax({
    	...
    	success: function(data1){
    		if(data1=="sessionTimeOut"){
	    		...
	    		$("#productBaseInfoDup").val(data.prices);
	    		$("#product_basePriceInfo").html(basePriceValue);
	    		$("#product_priceInfo").html("");
...
function goToOfferPage(path){
	try{
		
		//window.location.href=path+"/salescenter/offer";
		<%request.getSession().setAttribute("gotoofferpage","yes"); %>
		var action = path+"/salescenter/offer";
...
function goToUtilityOfferPage(){
	var path =$("input#contextPath").val();
	var customerId = $("input#customerId").val();
	var orderId = $("input#orderId").val();
	<%request.getSession().setAttribute("callTimeBeforeUtility",Double.valueOf(java.util.Calendar.getInstance().getTimeInMillis())); %>
					...
					<section id="contentfullcont">
						<header id="content_header">
							<div class="row">
								...
								<span class="cell pageTitle"><fmt:message key="recommendations.header"/></span>
						...
						<section id="content">
					        ...
					        <input type="hidden" id="categoryName" value="powerPitch" />
					        ...
					        <input type="hidden" id="isRecommendation" name="isRecommendation" value="true" />
							...
							<div class="contentwrapper">
								...
								<input type="hidden" id="pageTitle" name="pageTitle" value="Recommendations">
									<div class="leftbtns" style="width: 33%" >
									...
									<div class="SaversOffertbtns" align="center" style="width: 33%">
											<span>
												  <c:if test="${isUtilityOfferCompleted eq false}">
													  <input class="addtoorderbtn" type="button" id="UtiltyOffer" value="Utility Offer" onclick="goToUtilityOfferPage()"/>
												   </c:if>
										    </span>
										    <span>
											     <c:if test="${isSaverOfferCompleted eq false}">
											        <input class="addtoorderbtn" type="button" id="goToOffer" value="Savers Offer" onclick="goToOfferPage('<%=request.getContextPath()%>')"/>
										         </c:if>
										    </span>
									    
									</div>
									
									<div class="rightbtns" style="width: 33%">
									     <span>
										    <input type="button" id="backToDiscoveryId" name="backToDiscoveryId" value="< Back" onclick="backToDiscovery();" style="float:right;"/>
										 </span>
									</div>
								</div>
							</div>
										...
										<div class="pdet_content_block">
											...
											<div class="pdet_promos_features">
												...
												<input type="hidden" id="providerId" value="">
													<div class="pdet_promos_tab" id="promotionsTab" >Promotions</div>
													<div class="pdet_features_tab" id="featuresTab" >Highlights</div>
												</div>
