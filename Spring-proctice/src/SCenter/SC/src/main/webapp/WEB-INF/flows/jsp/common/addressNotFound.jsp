...
var states = JSON.parse(unescapeHTML('<c:out value="${states}" />'));
var unitType =JSON.parse(unescapeHTML('<c:out value="${unitType}" />'));
var serviceAddressType =JSON.parse(unescapeHTML('<c:out value="${serviceAddressType}" />'));
var rentOwn = JSON.parse(unescapeHTML('<c:out value="${rentOwn}" />'));
...
function unescapeHTML(p_string)
{
    ...
    {
        return p_string.replace(/&#34;/g,'"').replace(/&#034;/g,'"');
    }
...
function goToConfirm(){
	...
	*/
	...
	if($('input#zipOnlyCheckBoxId').prop("checked"))
	{
	...
	if(stat == false || !poStat)
	{
		if(!stat && !poStat){
		$("span#validatorMsg").css('display','block');
		$("span#validatorMsgs").html('*Required Fields <BR>"P O Box" Address not allowed.');
			
		}else if(!stat){
		$("span#validatorMsg").css('display','block');
		$("span#validatorMsgs").html('*Required Fields');
		}else if(!poStat){
			$("span#validatorMsg").css('display','block');
			$("span#validatorMsgs").html('"P O Box" Address not allowed.');
		}
...
var addressLineForZipOnly = '<c:out value="${addressLine1}" />';
var unitNumberForZipOnly = '<c:out value="${unitNumber}" />';
var cityForZipOnly = '<c:out value="${city}" />';
var zipForZipOnly = '<c:out value="${zip}" />';
...
function goToZipCodeOnly()
{
	...
	<section id="contentfullcont">
						...
						<section id="content">
							<form id="basicInfo" name="basicInfo" action="${flowExecutionUrl}" method="post" autocomplete="off">
							<input type="hidden"  id="contextPath" value="<%=request.getContextPath()%>" />
							<input type="hidden" id="prefix" name="prefix" value="${Prefix}">
							<input type="hidden" id="suffix" name="suffix" value="${Suffix}">
							<input type="hidden" id="firstName"  name="firstName" value='<c:out value="${firstName}" />'>
							<input type="hidden" id="middleName"  name="middleName" value='<c:out value="${middleName}" />'>
							<input type="hidden" id="lastName" name="lastName" value='<c:out value="${lastName}" />'>
							<input type="hidden" id="datepicker1"  name="datepicker1" value= '<c:out value="${datepicker1}" />'>
							<input type="hidden" id="datepicker2"  name="datepicker2" value='<c:out value="${datepicker2}" />'>
							<input type="hidden" id="datepicker3"  name="datepicker3" value='<c:out value="${datepicker3}" />'>
							<input type="hidden" id="unitTypevalue"   value='<c:out value="${unitTypevalue}" />'>
							<input type="hidden" id="statevalue"   value='<c:out value="${state}" />'>
							<input type="hidden" id="dwellingTypeValue"   value='<c:out value="${dwellingType}" />'>
							<input type="hidden" id="rentown"   value='<c:out value="${rentown}" />'>
							...
							<input type="hidden" id="_eventId" name="_eventId" value="addressNotFoundEvent">
							<input type="hidden" id="moveindateYesNo" name="moveindateYesNo" value="${moveInValue}">
											...
											<fieldset>
												...
												<div class="fieldsrow">
													<div class="addressfield">
														<input id="address1" name="address1" type="text" class="text addressfield" value='<c:out value="${addressLine1}"  />' title="Address 1" placeholder="Address 1"/>
													</div>
												</div><!-- Row -->
												<div class="fieldsrow">
													...
													<div class="unitnofield">
														<input id="unitNumber" name="unitNumber" type="text" class="text" value='<c:out value="${unitNumber}" />' title="Unit Number" size="10" placeholder="Unit Number" />
													</div>
												</div><!-- Row -->
												<div class="fieldsrow">
													<div class="cityfield">
														<input id="city" name="city" type="text" class="text" value='<c:out value="${city}" />' maxlength="100" title="City"  placeholder="City"/>
													</div>
													...
													<div class="zipfield">
														<input id="zipcode" name="zipcode" type="text" class="text" value='<c:out value="${zip}" />' maxlength="10" title="Zip Code" placeholder="Zip Code"/>
												...
												<span class="row" id="validatorMsg" style="display:none;color:#F00;" >
													<span class="cell" id="validatorMsgs" style="font-weight: bold; text-align: left;"></span>
												</span>			
											</fieldset>									
											<div class="rightbtns">
												<input  type="submit" id="addressNotFoundSubmit" name="addressNotFoundSubmit"   value="Try Again" onclick="return goToConfirm();"/>
											</div>	
										</div>
										<br>
