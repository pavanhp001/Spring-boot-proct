...
function backToQualification(flowEventId){
	...
	$("#discovery input[id='isBack']").val("true");
 ...
 */
	...
	{
		$('#existingProvidersSpan').after($('<span class="missingFields">* Required</span>'));
	} */
}
</script>
    ...
    border-radius: 3px;
					...
					<section id="contentfullcont">
						...
						<section id="content">
							...
							<input type="hidden" id="isOtherChecked" name="isOtherChecked" value="">
							...
							<div class="contentwrapper">
								...
								<div class="bottombuttons">
									<div class="rightbtns">
										<c:set var="disableBackButton" value="false" />
										<c:forEach var="selectedProvidersStatusMap" items="${selectedExistingCustomerStatusMap}">
											<c:if test="${selectedProvidersStatusMap.value eq 'disable'}">
												<c:set var="disableBackButton" value="true" />
											</c:if>
										</c:forEach>	
										<c:if test="${disableBackButton eq 'false'}">
