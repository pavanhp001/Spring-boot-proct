...
var isProductServiceCallCompleted = false;
var checkProdcutSrcInterval = '';
$(document).ready(function(){
	var dialogue = "Address Matched<br/>Please wait while we search for providers";
	$("#dialogue_content").html(dialogue);
	$("#addressFoundForward").hide();
	var pollTimeOut = parseInt("${pollTimeOut}");
	console.log("pollTimeOut"+pollTimeOut);

	// Initialize  interval for check ProductServiceCall is completed
	 checkProdcutSrcInterval = setInterval(function(){ 
		checkProductServiceCallIsCompleated(); 
	}, 500);

	//Set pollTimeOut to clearInterval of checkProductServiceCallIsCompleated()
	setTimeout(function(){ 
	clearInterval(checkProdcutSrcInterval); 
	$("#isProductServiceCallCompleted").val(isProductServiceCallCompleted);
	$("#dialogue_content").html("Address Matched<br/>Proceed to the next screen<br/><br/>");
	$("#addressFoundForward").show();
	}, pollTimeOut);
});
...
//Ajax Call for check ProductServiceCall is completed
function checkProductServiceCallIsCompleated(){
	var path = $("#contextPath").val();
	var url = path+"/salescenter/isProductServiceCallCompleted";
	try{    
		$.ajax({
			type: 'GET',
			url: url,
...
function onSuccess(data){
	isProductServiceCallCompleted = data;
	$("#isProductServiceCallCompleted").val(data);
	if(data == true){
		if(checkProdcutSrcInterval != ''){
			clearInterval(checkProdcutSrcInterval); 
		}
		$("#dialogue_content").html("Address Matched<br/>Proceed to the next screen<br/><br/>");
		$("#isProductServiceCallCompleted").val(data);
					...
					<section id="contentfullcont">
						<header id="content_header">
							<div class="row">
								...
								<span class="cell pageTitle">Valid Address</span>
						...
						<section id="content">
							...
							<input type="hidden" id="pageTitle" name="pageTitle" value="Valid Address">
							<div class="contentwrapper">
								...
								<div class="bottombuttons">
									<div class="rightbtns">
										<input type="button" id="addressFoundForward" name="addressFoundForward" value="Forward >" onclick="goToConfirm();"/>
									</div>
								</div>
							</div>
							<input type="hidden"  id="contextPath" value="<%=request.getContextPath()%>" />
							<input type="hidden" id="_eventId" name="_eventId" value="addressFoundEvent">
							...
							<input type="hidden" id="isProductServiceCallCompleted" value="false" name="isProductServiceCallCompleted" />
							</form>
						</section><!--  Content -->
