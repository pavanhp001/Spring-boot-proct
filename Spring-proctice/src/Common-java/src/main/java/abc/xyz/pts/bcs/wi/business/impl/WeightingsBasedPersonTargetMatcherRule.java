 ...
 * {@link TargetMatcher} strategy should be Weightings based name matcher.
 * </p>
 * 
 * @author sai.krishnamurthy
 */
public class WeightingsBasedPersonTargetMatcherRule implements Rule<TargetMatchRequest> {

	@Override
	public boolean evaluate(final TargetMatchRequest input) {
		return (	isActiveSearch(input) && 
					isNonIIRMatch(input) && 
					isNameFieldPresent(input) && 
					StringUtils.isBlank(input.getTargetItemToBeMatched().getFileReferenceNumber()));
	}

	private boolean isNonIIRMatch(final TargetMatchRequest input) {
		return !input.isIirEnabled() && TargetRequestType.TO_MATCH == input.getTargetItemToBeMatched().getTargetRequestType();
	}
